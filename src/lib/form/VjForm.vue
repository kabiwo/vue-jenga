<template>
  <el-form ref="formRef" label-width="auto" label-suffix="：" :model :rules :validate-on-rule-change="false"
    ...
    <div class="vj-grid vj-w-full" :class="[]" :style="{
      gridTemplateColumns: `repeat(${props.col || 1}, 1fr)`
    }">
      <div v-for="item in forms" :key="item.code">
        <VjFormItem :slots v-model="model[item.code]" :model :rootModel="props.rootModel || model" :addItemRule
...
import { computed, nextTick, ref, useSlots, watch, type Slot, type Slots } from 'vue';
import type { ElFormInsType, ItemForProps, VjFormItemProps, VjFormPropsTotal } from '.';
...
const forms = computed(() => {
  ...
  (props.form || []).forEach((item) => {
    ...
    if (!hide) {
      let it = item as ItemForProps;
      if (it.type === "for") {
        arr = arr.concat((typeof it.forItems === "function" ? it.forItems(it, model.value) : it.forItems) as VjFormItemProps[]);
      } else {
        arr.push(item);
      }
    }
